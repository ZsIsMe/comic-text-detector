...
「字級計算方法」提供兩個互相獨立的選項，預設為「OCR 對齊逐字計算」：

- `OCR 對齊逐字計算`：以 mit48 整行 OCR 對齊可靠文字，排除不可靠與異常字級，再依可靠結果的範圍建立候選字級。可靠字元不設最低數量，只有一個通過過濾的字元也會計算；若高度重疊的空區塊旁有已成功校準的同向區塊，空區塊會沿用其字級。
- `單字框計算`：沿用原有遮罩切分單字框與段落字級估算，不執行 mit48 OCR。

點擊「生成/更新 CTD」時會設定預設字級與 Step（首次預設為 `24.0, 2.0`）。兩種方法只共用這組候選設定；完成後會在 `ctd/measure.json` 記錄實際使用的方法。切換單選本身不修改文件，只有本次生成與字級計算全部成功後才保留新結果；失敗時會恢復執行前的 `measure.json`、`measure.debug.json` 與 `measure_ocr.json`。`font_size` 保留一位小數。
